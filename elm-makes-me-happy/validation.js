...
Elm.Validation = Elm.Validation || {};
Elm.Validation.make = function (_elm) {
   "use strict";
   _elm.Validation = _elm.Validation || {};
   if (_elm.Validation.values) return _elm.Validation.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   var update = F2(function (action,model) {
      var _p0 = action;
      if (_p0.ctor === "Email1Changed") {
            return _U.update(model,{email1: _p0._0});
         } else {
            return _U.update(model,{email2: _p0._0});
         }
   });
   var Email2Changed = function (a) {    return {ctor: "Email2Changed",_0: a};};
   var Email1Changed = function (a) {    return {ctor: "Email1Changed",_0: a};};
   var tickImage = function (isvalid) {
      var _p1 = isvalid;
      switch (_p1.ctor)
      {case "Empty": return "blank.png";
         case "Valid": return "tick.png";
         default: return "cross.png";}
   };
   var enabledAttr = function (enabled) {    return A2($Debug.watch,"enabled",enabled) ? _U.list([]) : _U.list([$Html$Attributes.disabled(true)]);};
   var contentToValue = function (str) {    return Email1Changed(A2($Debug.watch,"str",str));};
   var row = F5(function (address,caption,val,valid,action) {
      return A2($Html.div,
      _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "padding",_1: "0.3em"},{ctor: "_Tuple2",_0: "display",_1: "block"}]))]),
      _U.list([$Html.text(caption)
              ,A2($Html.input,
              _U.list([$Html$Attributes.value(A2($Debug.watch,"me1",val))
                      ,A3($Html$Events.on,"input",$Html$Events.targetValue,function (str) {    return A2($Signal.message,address,action(str));})]),
              _U.list([]))
              ,A2($Html.img,
              _U.list([$Html$Attributes.src(tickImage(valid))
                      ,$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "width",_1: "0.8em"}
                                                      ,{ctor: "_Tuple2",_0: "height",_1: "0.8em"}
                                                      ,{ctor: "_Tuple2",_0: "margin-top",_1: "0.3em"}
                                                      ,{ctor: "_Tuple2",_0: "margin-left",_1: "0.2em"}]))]),
              _U.list([]))]));
   });
   var model = {email1: "",email2: ""};
   var Model = F2(function (a,b) {    return {email1: a,email2: b};});
   var Invalid = {ctor: "Invalid"};
   var Valid = {ctor: "Valid"};
   var Empty = {ctor: "Empty"};
   var validEmail = function (string) {    return _U.eq(string,"") ? Empty : A2($String.contains,"@",string) ? Valid : Invalid;};
   var submitEnabled = function (model) {    return _U.eq(validEmail(model.email1),Valid) && _U.eq(model.email1,model.email2);};
   var view = F2(function (address,model) {
      return A2($Html.div,
      _U.list([]),
      _U.list([A5(row,address,"Enter email",model.email1,validEmail(model.email1),Email1Changed)
              ,A5(row,address,"Email again",model.email2,_U.eq(model.email2,"") ? Empty : _U.eq(model.email1,model.email2) ? Valid : Invalid,Email2Changed)
              ,A2($Html.div,
              _U.list([]),
              _U.list([A2($Html.button,_U.list([$Html$Attributes.disabled($Basics.not(submitEnabled(model)))]),_U.list([$Html.text("Submit")]))]))]));
   });
   var main = $StartApp$Simple.start({model: model,view: view,update: update});
   return _elm.Validation.values = {_op: _op
                                   ,main: main
                                   ,Empty: Empty
                                   ,Valid: Valid
                                   ,Invalid: Invalid
                                   ...
                                   ,view: view
                                   ,row: row
                                   ,contentToValue: contentToValue
                                   ,enabledAttr: enabledAttr
                                   ,submitEnabled: submitEnabled
                                   ,validEmail: validEmail
                                   ,tickImage: tickImage
                                   ,Email1Changed: Email1Changed
                                   ,Email2Changed: Email2Changed
                                   ,update: update};
};
