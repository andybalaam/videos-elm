...
Elm.Validation.make = function (_elm) {
   ...
   var tickImage = function (enabled) {    return enabled ? "tick.png" : "cross.png";};
   var validEmail = function (string) {    return A2($Debug.watch,"cont",A2($String.contains,"@",string));};
   var submitEnabled = function (model) {    return validEmail(model.email1) && _U.eq(model.email1,model.email2);};
   ...
   var row = F5(function (address,caption,val,enabled,action) {
      ...
      _U.list([$Html.text("Enter email:")
              ...
              _U.list([]))
              ,A2($Html.img,
              _U.list([$Html$Attributes.src(tickImage(enabled))
                      ,$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "height",_1: "0.8em"}
   ...
   var view = F2(function (address,model) {
      ...
      _U.list([A5(row,address,"Enter email",model.email1,validEmail(model.email1),Email1Changed)
              ,A5(row,address,"Email again",model.email2,_U.eq(model.email1,model.email2),Email2Changed)
              ,A2($Html.div,
              _U.list([A2($Html$Attributes.attribute,"disabled",submitEnabled(model) ? "false" : "true")]),
              _U.list([A2($Html.button,_U.list([]),_U.list([$Html.text("Submit")]))]))]));
   });
