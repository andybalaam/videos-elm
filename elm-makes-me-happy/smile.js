...
Elm.Smile = Elm.Smile || {};
Elm.Smile.make = function (_elm) {
   "use strict";
   _elm.Smile = _elm.Smile || {};
   if (_elm.Smile.values) return _elm.Smile.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Color = Elm.Color.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   var _op = {};
   var angle = function (i) {    return (0 - $Basics.pi) * i / 20;};
   var point = F3(function (radius,smileyness,i) {
      var theta = angle(i);
      return {ctor: "_Tuple2",_0: radius * $Basics.cos(theta),_1: smileyness * radius * $Basics.sin(theta)};
   });
   var face = F2(function (radius,smileyness) {
      return A3($Graphics$Collage.collage,
      200,
      200,
      _U.list([A2($Graphics$Collage.traced,
              _U.update($Graphics$Collage.defaultLine,{width: 5,color: $Color.red,join: $Graphics$Collage.Smooth}),
              $Graphics$Collage.path(A2($List.map,A2(point,radius,smileyness),_U.range(0,20))))
              ,A2($Graphics$Collage.move,{ctor: "_Tuple2",_0: -17,_1: 30},A2($Graphics$Collage.filled,$Color.black,$Graphics$Collage.circle(8)))
              ,A2($Graphics$Collage.move,{ctor: "_Tuple2",_0: 17,_1: 30},A2($Graphics$Collage.filled,$Color.black,$Graphics$Collage.circle(8)))]));
   });
   var scene = F2(function (_p1,_p0) {
      ...
      var _p5 = _p3._0;
      var _p4 = _p3._1;
      var yd = $Basics.toFloat(_p2._1) - $Basics.toFloat(_p4) / 2;
      var xd = $Basics.toFloat(_p2._0) - $Basics.toFloat(_p5) / 2;
      var smileyness = A3($Basics.clamp,-1,1,1 - 2 * ($Basics.sqrt(Math.pow(xd,2) + Math.pow(yd,2)) / (0.5 * $Basics.toFloat(A2($Basics.max,_p5,_p4)))));
      return A2(face,20,smileyness);
   });
   var main = A3($Signal.map2,scene,$Mouse.position,$Window.dimensions);
   return _elm.Smile.values = {_op: _op,main: main,scene: scene,face: face,point: point,angle: angle};
};
